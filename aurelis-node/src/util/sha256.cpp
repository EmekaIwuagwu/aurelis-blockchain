...
void SHA256::Update(const uint8_t* val, size_t len) {
...
std::string SHA256::HashToString(const std::string& input) {
    SHA256 ctx;
    ctx.Update(input);
    uint8_t digest[32];
    ctx.Final(digest);
...
std::vector<uint8_t> SHA256::Hash(const std::vector<uint8_t>& input) {
    SHA256 ctx;
